"""
Agent designed to extract new products from AppSumo's "New arrivals" collection.
"""
...
class AppSumoNewInput(BaseModel):
	"""User-provided parameters for the AppSumo extraction task."""

	url: AnyHttpUrl = Field(
		default=AnyHttpUrl('https://appsumo.com/collections/new/'),
		description='URL of the AppSumo "New arrivals" collection page',
	)
	...
	output_path: Path = Field(
		default=Path('appsumo_new_products.json'),
...
class AppSumoNewReport(BaseModel):
...
def _extract_badges(soup: BeautifulSoup) -> list[str]:
	"""Retrieve badge texts such as "Price increases in 4 days" or "Deal ends in 5 days"."""
	badges: list[str] = []
	for span in soup.select('div span'):
		...
		if not text:
			continue
		lower = text.lower()
		if any(keyword in lower for keyword in ('price', 'black friday', 'ending soon', 'deal ends')):
			badges.append(text)
	return badges
...
def _parse_html_sections(html_sections: list[str] | str, source_url: str) -> AppSumoNewReport | None:
	"""Parse HTML sections from the evaluate action to build AppSumoNewReport."""
...
async def run_appsumo_new_extraction(task_input: AppSumoNewInput) -> AppSumoNewReport | None:
	...
	try:
		...
		task = dedent(
			...
			- Pour chaque produit, capture:
			  ...
			  • badges (text tels que "Price increases in 4 days", "Deal ends in 5 days")
			...
			- Utilise `done` avec un objet AppSumoNewReport complet dans `data`.
			"""
		).strip()
		...
		agent = Agent(
			...
			llm=llm,
			browser=browser,
			output_model_schema=AppSumoNewReport,
			use_vision='auto',
		...
		if final_result:
			try:
				return AppSumoNewReport.model_validate_json(final_result)
			except ValidationError:
				match = re.search(r'```(?:json)?\s*(\{.*\})\s*```', final_result, re.DOTALL)
				if match:
					try:
						return AppSumoNewReport.model_validate_json(match.group(1))
					except ValidationError:
						return None
		return None
	finally:
...
def parse_arguments() -> AppSumoNewInput:
	"""Validate CLI arguments via Pydantic before launching the agent.

	Examples:
	--------
	python examples/histia/appsumo_new_extractor.py --max-products 50 --output appsumo.json
	"""

	parser = argparse.ArgumentParser(description='Extract products from AppSumo "New arrivals" page')
	parser.add_argument(
		'--url',
		default='https://appsumo.com/collections/new/',
		help='AppSumo collection URL (default: https://appsumo.com/collections/new/)',
	)
	parser.add_argument(
		'--max-products',
		type=int,
	...
	parser.add_argument(
		'--output',
		default='appsumo_new_products.json',
		help='Output JSON file path (default: ./appsumo_new_products.json)',
	)
	args = parser.parse_args()
	return AppSumoNewInput(url=AnyHttpUrl(args.url), max_products=args.max_products, output_path=Path(args.output))
...
async def main() -> None:
	...
	try:
		...
		report = await run_appsumo_new_extraction(task_input)
