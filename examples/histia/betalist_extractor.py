...
class BetalistStartup(BaseModel):
	...
	published_label: str | None = Field(None, description='Human readable label such as “Today” or “November 20th”.')
...
def _build_report_from_payloads(
	...
	for payload in payloads:
		...
		if not startup:
			...
			if skipped_parse == 1:
				...
				print(f'⚠️  Failed to parse first payload (id: {payload_id})')
			continue
	...
	if skipped_parse > 0:
		print(f'⚠️  Skipped {skipped_parse} entries due to parsing errors')
	if skipped_old > 0:
		print(f'⚠️  Skipped {skipped_old} entries older than cutoff date')
	if not startups:
		print(f'❌ No startups passed filtering (cutoff: {cutoff_date.isoformat()})')
		return None
	...
	for scroll_count in range(max_scrolls):
		...
		if oldest:
			if oldest <= cutoff_date:
				print(f'✅ Reached cutoff date: oldest={oldest.isoformat()}, cutoff={cutoff_date.isoformat()}')
				break
			if scroll_count % 10 == 0:
				print(f'📜 Scroll {scroll_count}: oldest date={oldest.isoformat()}, target={cutoff_date.isoformat()}')
		...
		if current_position == last_position:
			...
			if stalled_scrolls >= 3:
				print(f'⚠️  Scroll stalled after {scroll_count} attempts')
				break
		else:
			stalled_scrolls = 0
...
async def _extract_via_dom(browser: Browser, task_input: BetalistInput, cutoff_date: date) -> BetalistReport | None:
	page = await browser.get_current_page()
	if not page:
		print('⚠️  No active page for DOM extraction')
		return None

	payloads = await _evaluate_json_script(page, STARTUP_EXTRACTION_SCRIPT)
	if not payloads:
		print(f'⚠️  No startup payloads extracted from DOM (found {len(payloads)} entries)')
		return None

	print(f'✅ Extracted {len(payloads)} startup entries from DOM')
	...
	if report:
		print(f'✅ Built report with {len(report.startups)} startups after filtering')
	return report
...
async def run_betalist_extraction(task_input: BetalistInput) -> BetalistReport | None:
	...
	try:
		...
		print(f'📜 Scrolling until cutoff date: {cutoff_date.isoformat()}')
		...
		print('⚠️  DOM extraction failed, falling back to Agent-based extraction...')
		task = dedent(
			f"""
			Tu es déjà sur {source_url}. Ton objectif est d'extraire les startups apparues sur BetaList
			durant les {task_input.last_days} derniers jours (cutoff UTC: {cutoff_date.isoformat()}).

			Contraintes:
			- Ne navigue pas vers d'autres pages.
			- N'utilise pas `scroll` (le script a déjà chargé les données).
			- Utilise `evaluate` pour retourner un tableau JSON avec `outerHTML` de chaque carte startup.
			- Filtre toute carte dont la date (Today, Yesterday...) est antérieure à {cutoff_date.isoformat()}.
			- Limite la réponse à {task_input.max_startups} startups maximum.
...
async def main() -> None:
	try:
		...
		if report is None:
			print('❌ Aucun startup structuré retourné.')
			return
		...
		print(report_json)
		print(f'\n✅ Export sauvegardé dans: {task_input.output_path.resolve()}')
